...
import { RgthreeBetterTextWidget, RgthreeBetterButtonWidget } from './widgets.js'; // Import the necessary widgets from your code.

app.registerExtension({
    name: "iTools.testNode",
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name !== "iToolsTestNode") { return; }
        ...
        nodeType.prototype.onExecuted = function (message) {
            ...
            console.log("node Executed!", this)
        };
    ...
    async nodeCreated(node) {
        if (node.comfyClass !== "iToolsTestNode"){return;}

        if (allow_debug) {
            console.log("node", node);
        }
        
        if (node.size[1]< 105) {node.size[1] = 105;}
        ...
        txt.mouse = function(event, pos, node) {
            ...
            if (event.type == "pointerdown") {
                canvas.prompt("Label", txt.value, (v) => {
                    txt.value = v;
