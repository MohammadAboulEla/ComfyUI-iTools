...
// classes and functions here created by rgthree I partially edit some of them
...
export function drawLabelAndValue(ctx, label, value, width, posY, posX, height, options) {
    var _a;
    ...
    const labelX = posX + outerMargin + innerMargin + ((_a = options === null || options === void 0 ? void 0 : options.offsetLeft) !== null && _a !== void 0 ? _a : 0);
    ctx.fillText(label, labelX, midY);
    const valueXLeft = labelX + ctx.measureText(label).width + 7;
    const valueXRight = posX + width - (outerMargin + innerMargin);
...
export function drawNodeWidget(ctx, options) {
    ...
    ctx.beginPath();
    ctx.roundRect(data.margin + data.posX, data.posY, data.width - data.margin * 2, data.height, lowQuality ? [0] : options.borderRadius ? [options.borderRadius] : [options.height * 0.5]);
    ctx.fill();
    if (!lowQuality) {
        ctx.stroke();
    }
...
// my widgets
export class RgthreeDoubleTextWidget {
    ...
    draw(ctx, node, width, y, height) {
        ...
        // Draw the first text widget
        const widgetData1 = drawNodeWidget(ctx, { width: halfWidth, height, posY: y, posX:0 });
        if (!widgetData1.lowQuality) {
            drawLabelAndValue(ctx, this.name1, this.value1, halfWidth, y, 0, height);
        }

        // Draw the second text widget (on the same row, second half)
        const widgetData2 = drawNodeWidget(ctx, { width: halfWidth + 15, height, posY: y, posX: halfWidth - 15});
        if (!widgetData2.lowQuality) {
            drawLabelAndValue(ctx, this.name2, this.value2, halfWidth + 15, y, halfWidth - 15 , height);
        }
    }
    ...
    mouse(event, pos, node) {
        ...
        // Handle pointerdown for the first text widget
        if (event.type === "pointerdown" && pos[0] < halfWidth) {
            canvas.prompt("Label 1", this.value1, (v) => (this.value1 = v), event);
            return true;
        }

        // Handle pointerdown for the second text widget
        if (event.type === "pointerdown" && pos[0] >= halfWidth) {
            canvas.prompt("Label 2", this.value2, (v) => (this.value2 = v), event);
            return true;
        }
...
// usage example
// import { RgthreeBaseWidget, RgthreeBetterButtonWidget, RgthreeDividerWidget } from "./rgthree_widgets.js";
// this.addCustomWidget(new RgthreeDividerWidget());
