...
def csv_special_to_yaml(csv_file, yaml_file):
    ...
    with open(csv_file, newline="", encoding="utf-8") as csvfile:
        ...
        for row in reader:
            ...
            # Construct the YAML format dictionary
            entry = {"name": name, "prompt": prompt, "negative_prompt": negative_prompt}
            result.append(entry)
        ...
        with open(category_file_path, "w", encoding="utf-8") as category_file:
            for entry in entries:
                ...
                yaml_entry = yaml.dump(
                    [entry], allow_unicode=True, default_flow_style=False
                ).strip()
                category_file.write(
                    yaml_entry + "\n\n"
                )  # Add an empty line after each entry
                ...
                try:
                    ...
                    if isinstance(yaml_data, list):
                        ...
                        for entity in yaml_data:
                            if "name" in entity:
                                entity["name"] = (
                                    f"{os.path.splitext(filename)[0]} | {entity['name']}"
                                )
